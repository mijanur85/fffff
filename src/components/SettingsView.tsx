...
interface Props {
  ...
  onChangeAnimationIntensity?: (intensity: 'low' | 'medium' | 'high') => void;
  ...
  return (
    <div className="space-y-6 pb-28 px-4 max-w-2xl mx-auto">
      ...
      <div className="space-y-3">
        ...
        <div className={`p-3 rounded-2xl border flex items-center justify-between ${theme.cardClass}`}>
          <div className="flex items-center gap-2.5">
            <div className="p-1.5 rounded-lg bg-amber-500/20 text-amber-400">
              <Zap className="w-4 h-4" />
            </div>
            <div>
              <h5 className="text-xs font-bold text-white">Performance / Battery Saver</h5>
              <p className="text-[10px] text-zinc-400">Reduce animation effects on low power</p>
            </div>
          </div>
          ...
          <button
            ...
            className={`w-10 h-6 rounded-full p-0.5 transition-colors ${
              settings.performanceMode ? 'bg-cyan-400' : 'bg-zinc-800'
            }`}
          >
            <div
              className={`w-5 h-5 rounded-full bg-black transition-transform ${
                settings.performanceMode ? 'translate-x-4' : 'translate-x-0'
              }`}
            />
          </button>
        </div>

        {/* Enable / Disable Animated Background Toggle */}
        <div className={`p-3 rounded-2xl border flex items-center justify-between ${theme.cardClass}`}>
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h5 className="text-xs font-bold text-white">Animated Backgrounds</h5>
              <p className="text-[10px] text-zinc-400">Toggle active background animations</p>
            </div>
          </div>

          <button
            onClick={() => onToggleAnimations?.(!(settings.animationsEnabled ?? true))}
            className={`w-10 h-6 rounded-full p-0.5 transition-colors ${
              (settings.animationsEnabled ?? true) ? 'bg-cyan-400' : 'bg-zinc-800'
            }`}
          >
            <div
              className={`w-5 h-5 rounded-full bg-black transition-transform ${
                (settings.animationsEnabled ?? true) ? 'translate-x-4' : 'translate-x-0'
              }`}
            />
          </button>
        </div>

        {/* Animation Intensity Selector (Low / Medium / High) */}
        {(settings.animationsEnabled ?? true) && (
          <div className={`p-3 rounded-2xl border space-y-2 ${theme.cardClass}`}>
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-white">Animation Intensity</span>
              <span className="text-[10px] font-mono text-cyan-400 uppercase font-extrabold">
                {settings.animationIntensity ?? 'medium'}
              </span>
            </div>
            <div className="grid grid-cols-3 gap-2 pt-1">
              {(['low', 'medium', 'high'] as const).map((level) => {
                const isActive = (settings.animationIntensity ?? 'medium') === level;
                return (
                  <button
                    key={level}
                    type="button"
                    onClick={() => onChangeAnimationIntensity?.(level)}
                    className={`py-1.5 rounded-xl border text-center text-xs font-bold capitalize transition-all ${
                      isActive
                        ? 'bg-cyan-400 text-black border-cyan-300 shadow-[0_0_10px_rgba(34,211,238,0.3)]'
